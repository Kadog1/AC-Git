...
Function getMandatory(sheet As Variant, header As Range) As String()
    ...
    For Each c In header
        If InStr(1, c.Value, "required") <> 0 Or InStr(1, c.Value, "Service related to the address") <> 0 Then
...
Sub exe()
    ...
    For Each sheet In wsTemplate.Worksheets
        ...
        If sheet.Name <> "basic_info" And sheet.Name <> "Inhalte" And sheet.Name <> "ISO" And sheet.Name <> "Summary" Then
            ...
            If sheet.Name = "Address check" Then
            
                fields = "Sequence number (automatic)|Service related to the address|Name of company (required)|Additional address information" & Chr(10) & "(optional)|Contact person (optional)||Street + house number/ P.O. Box (required)|Postcode" & Chr(10) & "(required)|City" & Chr(10) & "(required)|Country" & Chr(10) & "(required, ISO code if possible)|E-mail" & Chr(10) & "(optional)|If necessary comment"
    ...
    Else
        ...
        If bValid2 = False Then
            MsgBox ("Please make sure that in each row all fields, which are marked as 'required', are filled!")
        End If
    End If
...
Sub sendMail(wsTemplate As Variant, bValid As Boolean, bError As Boolean)
    ...
    If bValid Or bError Then
        ...
        'Vorgefertigter Text
        body = "Hello CAD Team, <br><br> Please find the completed template for Address comparison attached."
