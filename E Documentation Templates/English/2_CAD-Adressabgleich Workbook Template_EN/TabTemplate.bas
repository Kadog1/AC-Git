...
Sub createDetailWorksheet()
    ...
    For i = 1 To lastrow - 13
        ...
        If SheetExist(CStr(i)) = False Then
            ...
            Call getRegister(ActiveWorkbook.Worksheets("Register"), ActiveWorkbook.Worksheets(CStr(i)), ActiveWorkbook.Worksheets(CStr(i)).Cells(20, 4), ActiveWorkbook.Worksheets(CStr(i)).Cells(32, 4), 56, 11)
            ...
            For j = 5 To 7
                ...
                Select Case colorScheme
                    ...
                    Case 2
                        ...
                        If i <> 34 Then ws.Cells(i, 8) = "No deviation"
                
                    Case 3
                    
                        ws.Cells(i, j).Interior.Color = RGB(180, 198, 231)
                        If i <> 34 Then ws.Cells(i, 8) = "Insignificant deviation"
                 
                    Case 4
                    
                        If i <> 34 Then ws.Cells(i, j).Interior.Color = RGB(248, 203, 173)
                        If i <> 34 Then ws.Cells(i, 8) = "Significant deviation"
...
Sub EinzelValidierung_Click()
    ...
    AnswerYes = MsgBox("Would you like to finish the individual comparison? All buttons will be removed", vbQuestion + vbYesNo, "User Repsonse")
...
Function validateConclusion(ws As Worksheet, i As Long, j As Long) As Boolean
    ...
    If i <> 34 Then
        ...
        If ws.Cells(i, j).Interior.Color = RGB(0, 0, 0) Then
            If ws.Cells(i, 8) <> "No deviation" Then validateConclusion = False
        End If

        If ws.Cells(i, j).Interior.Color = RGB(180, 198, 231) Then
            If ws.Cells(i, 8) <> "Insignificant deviation" Then validateConclusion = False
        End If

        If ws.Cells(i, j).Interior.Color = RGB(248, 203, 173) Then
            If ws.Cells(i, 8) <> "Significant deviation" Then validateConclusion = False
        End If
...
Sub retrieveScreenshots()
    
    Dim idxAddress As String, orderNo As String, tsScreenshotCreated As Date, strTSScreenshotCreated As String, locationPath As String, FileName As String, pathPNG As String
    ...
    binfo = ws_basic_info.range("B1:B10").Value
    
    orderNo = CStr(binfo(1, 1))
    tsScreenshotCreated = ws_basic_info.range("E3").Value
    strTSScreenshotCreated = Format(tsScreenshotCreated, "yyyy-mm-dd hh:mm:00.000")
    
    listSkipSheets = Array("Start", "Summary", "Input Address data", "Input evaluation", "TabTemplate", "Team Approval Documentation", _
                    "TF_FIS", "TF_ok", "TF_X", "Register", "CPI Score", "basic_info", "Versandliste")
    ...
    For Each t_ws In wb.Worksheets
        ...
        If IsError(Application.Match(t_ws.Name, listSkipSheets, 0)) Then  ' not case sensitive
            ...
            SQL = "SELECT p.Company, a.Strasse_Hausnummer_Postfach, a.Stadt, a.Postleitzahl, a.Land," & vbCrLf
            SQL = SQL & "p.OrderNo, p.idxAddress, p.tsScreenshotCreated, s.locationPath, s.pngFileName, s.register FROM [CAD].[dbo].[tAC_ProdScreenshots] p" & vbCrLf
            SQL = SQL & "LEFT JOIN [CAD].[dbo].[tAC_Screenshots] s ON p.Company = s.Company AND s.tsScreenshotCreated = '" & strTSScreenshotCreated & "' AND p.OrderNo = '" & orderNo & "' AND p.idxAddress = '" & idxAddress & "'" & vbCrLf
            SQL = SQL & "LEFT JOIN [CAD].[dbo].[tAC_Addresses] a ON p.Company = a.Company AND p.OrderNo = '" & orderNo & "' AND p.idxAddress = '" & idxAddress & "'" & vbCrLf
            SQL = SQL & "WHERE p.OrderNo = '" & orderNo & "'" & vbCrLf
            SQL = SQL & "AND p.idxAddress = '" & idxAddress & "'" & vbCrLf
            SQL = SQL & "AND p.tsScreenshotCreated = '" & strTSScreenshotCreated & "'"
            ...
            If rsSQL.RecordCount > 0 Then
                ...
                t_ws.Shapes.AddPicture pathPNG, True, True, 40, 570, 960, 540
                
                ' Insert Address
                Dim insertCol As String
                insertCol = "E"
                If rsSQL.Fields("register") = "D&B Hoovers" Then
                    insertCol = "F"
                End If
                t_ws.range(insertCol & "25").Value = rsSQL.Fields("Company")
                t_ws.range(insertCol & "29").Value = rsSQL.Fields("Strasse_Hausnummer_Postfach")
                t_ws.range(insertCol & "30").Value = rsSQL.Fields("Postleitzahl")
                t_ws.range(insertCol & "31").Value = rsSQL.Fields("Stadt")
                t_ws.range(insertCol & "32").Value = rsSQL.Fields("Land")
            End If
        End If
    Next
