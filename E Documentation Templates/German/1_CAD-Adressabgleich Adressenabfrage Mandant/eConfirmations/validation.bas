...
Function getHeader(sheet As Variant) As Range
    ...
    Set objul = sheet.Range("B1:B100").Find(what:="laufende Nummer (automatisch)", LookIn:=xlValues)
    ...
    Set objur = Range(lcol & lrow).Resize(2, 50).Find("Falls notwendig Kommentar", LookIn:=xlValues)
...
Function getMandatory(sheet As Variant, header As Range) As String()
    ...
    For Each c In header
        If InStr(1, c.Value, "benötigt") <> 0 Or InStr(1, c.Value, "Debitor/Kreditor/Sonstige") <> 0 Or InStr(1, c.Value, "Art der Dienstleistung") <> 0 Or InStr(1, c.Value, "Mit der Adresse ver-bundene Dienstleistung") <> 0 Then
...
Sub exe()
    ...
    For Each sheet In wsTemplate.Worksheets
        ...
        If sheet.Name <> "basic_info" And sheet.Name <> "Inhalte" And sheet.Name <> "ISO" And sheet.Name <> "Summary" Then
            ...
            If sheet.Name = "Debitor_Kreditor_Sonst" Then
                fields = "laufende Nummer (automatisch)|Debitor/Kreditor/Sonstige|Konto/ Rechnungsnummer|Name der Firma" & Chr(10) & "(benötigt)|Adresszusatz|Ansprechpartner (optional)||Straße + Hausnummer/ Postfach (benötigt)|Postleitzahl" & Chr(10) & "(benötigt)|Stadt" & Chr(10) & "(benötigt)|Land (benötigt, falls möglich ISO Code)|E-Mail" & Chr(10) & "(optional)|Falls notwendig Kommentar"
                
            ElseIf sheet.Name = "Bank" Then
                fields = "laufende Nummer (automatisch)|Nummer des Hauptbuch-" & Chr(10) & "kontos (optional)|IBAN" & Chr(10) & "(optional)|Name der Bank" & Chr(10) & "(benötigt)|Adresszusatz|Ansprechpartner (optional)||Straße + Hausnummer/ Postfach (benötigt)|Postleitzahl" & Chr(10) & "(benötigt)|Stadt" & Chr(10) & "(benötigt)|Land (benötigt, falls möglich ISO Code)|E-Mail" & Chr(10) & "(optional)|Falls notwendig Kommentar"
                
            ElseIf sheet.Name = "Rechts-_Steuerberater" Then
                fields = "laufende Nummer (automatisch)|Art der Dienstleistung|Name der Kanzlei" & Chr(10) & "(benötigt)|Adresszusatz|Ansprechpartner (benötigt)||Straße + Hausnummer / Postfach (benötigt)|Postleitzahl" & Chr(10) & "(benötigt)|Stadt" & Chr(10) & "(benötigt)|Land (benötigt, falls möglich ISO Code)|E-Mail" & Chr(10) & "(optional)|Falls notwendig Kommentar"
                
            ElseIf sheet.Name = "Adresscheck" Then
                fields = "laufende Nummer (automatisch)|Mit der Adresse ver-bundene Dienstleistung|Name der Firma" & Chr(10) & "(benötigt)|Adresszusatz|Ansprechpartner (optional)||Straße + Hausnummer/ Postfach (benötigt)|Postleitzahl" & Chr(10) & "(benötigt)|Stadt" & Chr(10) & "(benötigt)|Land (benötigt, falls möglich ISO Code)|E-Mail" & Chr(10) & "(optional)|Falls notwendig Kommentar"
            ...
            For Each cell In hdr
                ...
                If cell.Value <> header(i) Then
                    ...
                    MsgBox ("Bitte beachten ändern Sie die Reihenfolge der Spalten wieder in den Ausgangszustand des Dokuments!")
                    Exit Sub
                End If
    ...
    If bValid3 = False Then
        MsgBox ("Es wurde keine Eingabe getätigt!")
    Else
    
        'problem with validity of values
        If bValid1 = False Then
            MsgBox ("Bitte überprüfen Sie in allen Sheets die Orange markierten Einträge!")
        End If
        
        'problem with mising obligatory entries
        If bValid2 = False Then
            MsgBox ("Bitte überprüfen sie, ob in jeder Zeile alle Felder die mit 'benötigt' markiert sind ausgefüllt sind!")
        End If
    End If
...
Function validate(hdr As Range, sheet As Variant, lastrow As Long) As Boolean
    ...
    For Each i In hdr
        If InStr(1, i, "Postleitzahl") <> 0 Then
...
'
    ...
    If validate And Not bIBAN Then
        answer = MsgBox("Eine der von Ihnen eingebenen IBANs scheint nicht korrekt zu sein, wollen Sie trotzdem senden?", vbQuestion + vbYesNo + vbDefaultButton2, "Fehler bei der IBAN")
...
Sub sendMail(wsTemplate As Variant, bValid As Boolean, bError As Boolean)
    ...
    If bValid Or bError Then
        ...
        If bError Then
            ...
            MsgBox "Es ist leider ein technischer Fehler aufgetreten. Dieses Template wurde dennoch auf Ihrem Desktop gespeichert und wird nun per E-Mail an adressabgleich@de.ey.com versendet. In Kürze wird sich das CAD-Team bei Ihnen melden." & vbCrLf & "Bitte entschuldigen Sie den Fehler und vielen Dank für Ihre Eingabe!"
    
            subject = "RE: ERROR Beauftragung Adressabgleich " & orderNo & " für " & clientName
         
        ElseIf bValid Then
        
            MsgBox "Dieses Template wurde erfolgreich auf Ihrem Desktop gespeichert und wird nun per E-Mail an adressabgleich@de.ey.com versendet." & vbCrLf & "Vielen Dank für Ihre Eingabe!"
        ...
        'Vorgefertigter Text
        body = "Hallo CAD Team, <br><br> anbei finden Sie das ausgefüllte Template für den Adressabgleich."
