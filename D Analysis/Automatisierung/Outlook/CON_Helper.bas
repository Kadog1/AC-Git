...
2             If LCase(Application.Session.Accounts.item(I)) = "anykey@de.ey.com" Then
3                 indexAccount = I
4                 Exit For
5             End If
6         Next
...
16        Next
          ...
          For I = 1 To c_mails
              ...
              If InStr(subject, "Beauftragung Adressabgleich") > 0 Or InStr(subject, "Order of debtor") > 0 Then
                  ...
                  Set rsOrderbook = Connection.Execute("SELECT * FROM tCON_Orderbook_TEST WHERE [OrderNo] = '" & OrderNo & "'") ' PROD / TEST
...
22                        If InStr(objAtt.FileName, "1_CAD-Adressabgleich Adressenabfrage Mandant") > 0 Or InStr(objAtt.FileName, "Debitoren Einzelposten") > 0 Then
                              ...
                              Call updateSQL("UPDATE tCON_Orderbook_TEST Set AC_Status = 'InputDataAvailable' WHERE OrderNo = '" & OrderNo & "'") ' PROD / TEST
                              Call updateSQL("UPDATE tCON_Orderbook_TEST Set tsInputDataAvailable = '" & timeNow & "' WHERE OrderNo = '" & OrderNo & "'") ' PROD / TEST
                          End If
...
16        Next
          ...
          For I = 1 To c_mails
              ...
              If InStr(subject, "CAD Adressabgleich") > 0 And InStr(subject, ",CON00") = 0 Then
                  ...
                  Dim yearend As String, client As String, engcontact As String, engpartner As String, engmanager As String, additionalCntct As String, gisid As String, Confirmation As String, engcode As String
                  ' Check, ob Ordernummer schon storniert
                 
                                  Dim querySQL As String, arapConfirmation As String
                                  arapConfirmation = "Kreditoren"
                                  querySQL = "SELECT * FROM tCON_Orderbook_TEST WHERE [OrderNo] = '" & OrderNo & "'" ' PROD / TEST
                                  If InStr(OrderNo, "AP000") > 0 Or InStr(OrderNo, "AR000") > 0 Then
                                        If InStr(OrderNo, "AR000") > 0 Then arapConfirmation = "Debitoren"
                                        querySQL = "SELECT oi.GISID, oi.GISName AS client, oi.EngCode AS engCode, '" & arapConfirmation & "' AS Confirmation, oh.PeriodEnd AS YearEnd, CASE WHEN oi.Storno = 0 THEN NULL ELSE 1 END AS Storno, NULL AS AC_Status, "
                  ...
                  If bSendToAuditi Then
                      ...
                      If mails(I).Attachments.Count > 0 Then
                          ...
                          Set objAtt = mails(I).Attachments.item(1)
22                        If InStr(objAtt.FileName, "CAD Adressabgleich") > 0 Then
                              ...
                              Dim timeNow As String
                              timeNow = Format(Now(), "yyyy-MM-dd hh:mm:ss")
                              Call updateSQL("UPDATE tCON_Orderbook Set AC_Status = 'InputDataReceived' WHERE OrderNo = '" & OrderNo & "'")
                              Call updateSQL("UPDATE tCON_Orderbook Set tsInputDataReceived = '" & timeNow & "' WHERE OrderNo = '" & OrderNo & "'")
                          End If
...
If LCase(Outlook.Application.Session.Accounts.item(I)) = "anykey@de.ey.com" Then ' PROD / TEST
    indexAccount = I
Exit For
End If
Next
...
For I = 1 To c_mails
    ...
    Set rsOrderbook = Connection.Execute("SELECT [AC_Status], [AC_Preparer], [Client], [GISID], [YearEnd] FROM tCON_Orderbook_TEST WHERE [OrderNo] = '" & OrderNo & "'") ' PROD / TEST
    Do While rsOrderbook.EOF = False
        ...
        ElseIf rsOrderbook("AC_Status") = "InputDataReceived" Or rsOrderbook("AC_Status") = "InProgress" Or rsOrderbook("AC_Status") = "ReadyForReview" Then
            ' Move "* 1_CAD-Adressabgleich Adressenabfrage Mandant *.xlsx" to PrevFiles & Save "* 1_CAD-Adressabgleich Adressenabfrage Mandant *.xlsx"
            ...
            If Len(fileInputSheet) > 0 Then
                ...
                DestinFileName = saveFolder & "Previous InputDatenSheets\" & fileInputSheet
...
Public Function auditiCreateInputSheet(t_template As String) As String
saveFolder = "\\Defrnappfl101.ey.net\101fra00010\T\TCC_SB\Z_Archive\eConfirmations\Datenbank\C Workplace\" & OrderNo & "\"
